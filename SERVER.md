...
|-----------------|------|----------|
| **Frontend** (nginx в контейнере) | **3000** | Веб-интерфейс; внутри контейнера `/api` проксируется на backend |
...
| Файл | Кто публикует `:8000` | Кто публикует `:3000` |
|------|------------------------|-------------------------|
...
1. **На машине уже занят 8000 или 3000** (другой сайт, старый контейнер):
...
2. **Смена портов на хосте** — в `docker-compose.yml` замените, например:
   ...
   ```yaml
     - "3001:3000"
   ```
   Внутри контейнеров порты **8000** и **3000** не меняются — только левая часть (`хост:контейнер`).
...
Для продакшена лучше поставить **nginx/Caddy** на 443 и проксировать на `127.0.0.1:3000`, а порты 3000/8000 не светить в интернет.
...
Порты на хосте те же: **3000** (сайт), **8000** (API). Фронт использует `frontend/nginx.vpn.conf` и шлёт `/api` на хост **`gluetun:8000`** внутри сети Docker.
