...
**Проверка:** откройте `http://ваш-сервер:8000/health` или `http://ваш-сервер:3000/api/health`. В ответе должно быть `"aiConfigured": true`. Если `false`:
...
Если при генерации появляется «Сервер не может связаться с api.groq.com» — на сервере нет доступа в интернет до api.groq.com (файрвол или сеть).
